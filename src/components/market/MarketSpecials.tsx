      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {specials.map((special) => {
            const isIphone = special.id === 101;
            
            return (
              <div 
                key={special.id} 
                className={`bg-white p-4 md:p-6 rounded-lg shadow-md border border-gray-200 transform transition duration-300 hover:scale-105 hover:shadow-xl ${isIphone ? 'ring-2 ring-[#721244]' : ''}`}
                data-aos="fade-up"
              >
                <div className="aspect-square mb-4 relative overflow-hidden rounded-lg">
                  ...
                  />
                  {isIphone && (
                    <div className="absolute top-2 right-2 bg-[#721244] text-white text-xs font-bold py-1 px-2 rounded-full">
                      Featured
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <BuyNowButton productId={special.id} className="w-full" />
                </div>
              </div>
            );
          })}
